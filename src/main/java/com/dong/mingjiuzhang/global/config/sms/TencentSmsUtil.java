...
//            httpProfile.setProxyPort(port);
     ...
     */
    public static SendSmsResponse sendSms(String phoneNumber, SmsTemplateEnum smsTemplateEnum, String session, String... templateParams) {
        String[] phoneNumbers = {phoneNumber};
        return sendSms(phoneNumbers, smsTemplateEnum, session, templateParams);
    }
     ...
     * @param templateParams
     */
    public static SendSmsResponse sendSms(String[] phoneNumbers, SmsTemplateEnum smsTemplateEnum, String session, String[] templateParams) {
        try {
            // 获取SmsClient
            ...
            req.setSmsSdkAppid(SmsConfig.SDK_APP_ID);
            ...
            String templateID = smsTemplateEnum.getTemplateID();
            req.setTemplateID(templateID);

            /* 下发手机号码，采用 e.164 标准，+[国家或地区码][手机号]
             * 例如+8613711112222， 其中前面有一个+号 ，86为国家码，13711112222为手机号，最多不要超过200个手机号*/
            for (int i = 0; i < phoneNumbers.length; i++) {
                if (!phoneNumbers[i].startsWith(CN_CODE)) {
                    phoneNumbers[i] = CN_CODE + phoneNumbers[i];
                }
            }
            ...
            // 输出 JSON 格式的字符串回包
            String responseResult = SendSmsResponse.toJsonString(res);
    ...
    public static void parseResponseResult() {
    ...
    public static void main(String[] args) {
        String[] phoneNumbers = {"+8618657186958"};
        String[] templateParams = {"5678"};
        String session = "{userId:1}";
        // 发送短信
//        sendSms(phoneNumbers, templateParams, SmsTemplateEnum.ORDER, session);
